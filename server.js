...
// Render diskinde sadece geçici dosya tutulur.
// Kalıcı medya Supabase Storage'a gönderilir.
...
app.use(
  session({
    secret:
      ...
      "aevix-development-secret-change-me",
...
// Eski local dosyalar için geriye dönük destek.
app.use(
  "/uploads",
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error: "Tüm alanları doldur ve şifreleri aynı gir."
      });
    }
    ...
    if (!/^[a-zA-Z0-9_]{3,24}$/.test(username)) {
      return res.status(400).json({
        error:
          "Kullanıcı adı 3-24 karakter olmalı; sadece harf, sayı ve _ kullanılabilir."
      });
    }
...
async function ensureSupabaseBucket() {
  if (!SUPABASE_URL) {
    throw new Error(
      "SUPABASE_URL Render Environment Variables bölümünde eksik."
    );
  }

  if (!SUPABASE_KEY) {
    throw new Error(
      "SUPABASE_SERVICE_ROLE_KEY Render Environment Variables bölümünde eksik."
    );
  }
  ...
  throw new Error(
    `Supabase bucket oluşturulamadı: ${text}`
  );
}
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "Müzik alanına sadece ses dosyası yükleyebilirsin."
        });
      }
